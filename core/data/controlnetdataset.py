...
@DatasetRegistry.register('controlnetdataset')
class ControlNetDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        prompt_emb,pooled_prompt_embeds,text_ids = text_embeding["prompt_embeds"][0],text_embeding['pooled_prompt_embeds'][0],text_embeding['text_ids']
        return {
    ...
    def collate_fn(examples):
        pixel_values = torch.stack([example["target_image"] for example in examples])
        pixel_values = pixel_values.to(memory_format=torch.contiguous_format).float()

        conditioning_pixel_values = torch.stack([example["cond_image"] for example in examples])
        conditioning_pixel_values = conditioning_pixel_values.to(memory_format=torch.contiguous_format).float()

        prompt_ids = torch.stack([example["prompt_emb"] for example in examples])
